...
export default function ContactosAsesorPage() {
  ...
  return (
    <LayoutDashboard>
      ...
      <div className="flex flex-wrap justify-between gap-3 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Consulta contactos por empresa, llama al cliente y crea nuevos encargados vinculados a la empresa.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[420px_1fr] gap-4 mb-5">
        <section className="bg-white border rounded-2xl p-4">
          <h2 className="text-sm font-bold text-gray-800 mb-1">Buscar empresa</h2>
          <p className="text-xs text-gray-500 mb-3">Busca por razón social, sede o NIT para crear contactos vinculados.</p>
          ...
          {selectedEmpresa && (
            <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-3">
              ...
              {selectedEmpresa.empresaDireccion && (
                <p className="text-xs text-gray-500 mt-1">{selectedEmpresa.empresaDireccion}</p>
              )}
          ...
          <div className="mt-4 rounded-xl border border-gray-100 overflow-hidden">
            <div className="px-3 py-2 bg-gray-50 border-b border-gray-100 flex items-center justify-between">
              <p className="text-xs font-bold text-gray-600">Contactos de la empresa</p>
              {loadingEmpresaContactos && <span className="text-[10px] text-gray-400">Cargando...</span>}
            </div>
            {!selectedEmpresa ? (
              <p className="p-4 text-xs text-gray-400">Selecciona una empresa para ver y crear sus contactos.</p>
            ) : empresaContactos.length === 0 ? (
              <div className="p-4 text-center">
                <p className="text-xs text-gray-400 mb-3">Esta empresa aún no tiene contactos activos.</p>
                <button
                  type="button"
                  onClick={() => openCreate(selectedEmpresa)}
                  className="px-3 py-2 rounded-lg bg-[#1C355E] text-white text-xs font-bold"
        ...
        <section>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
            ...
            <Card label="Empresas esta página" value={items.length} loading={loading} />
            <Card label="Contactos esta página" value={totalContactosPagina} loading={loading} />
          </div>

          <div className="bg-white border rounded-2xl overflow-hidden">
            <div className="p-4 border-b border-gray-100 flex flex-wrap gap-3 items-center justify-between">
              <div>
                <h2 className="text-sm font-bold text-gray-800">Empresas con contactos guardados</h2>
                <p className="text-xs text-gray-400">Vista general como admin, disponible para asesores.</p>
              </div>
              <input
                type="text"
                placeholder="Buscar por empresa, NIT, contacto, cargo o teléfono..."
                value={search}
            ...
            {error && <div className="p-4 bg-red-50 text-red-600 text-sm font-semibold">{error}</div>}
            {success && <div className="p-4 bg-emerald-50 text-emerald-700 text-sm font-semibold">{success}</div>}

            {loading && items.length === 0 ? (
              <div className="text-center py-10 text-gray-500 text-sm">Cargando contactos...</div>
            ) : items.length === 0 ? (
              <div className="text-center py-10 text-gray-500 text-sm">
                {searchDebounced ? "No hay coincidencias con la búsqueda." : "Aún no hay contactos registrados."}
              </div>
            ) : (
              <div className="divide-y divide-gray-100">
                {items.map((empresa) => {
                  const contactos = Array.isArray(empresa.contactos) ? empresa.contactos : [];
                  const isOpen = Boolean(openByClient[empresa.clienteId]);
                  return (
                    <div key={empresa.clienteId} className="p-4">
                      <div className="flex flex-wrap gap-3 justify-between">
                        <button
                          type="button"
                          className="min-w-0 flex-1 text-left"
                          onClick={() => setOpenByClient((prev) => ({ ...prev, [empresa.clienteId]: !isOpen }))}
                        >
                      ...
                      {isOpen && (
                        <div className="mt-3 overflow-x-auto">
                          <table className="w-full text-sm min-w-[760px] border border-gray-100 rounded-xl overflow-hidden">
                            ...
                            <tbody>
                              {contactos.map((ct, i) => (
                                <ContactRow
                                  ...
                                  contacto={ct}
                                  empresa={empresa}
                                  onEdit={() => openEdit(empresa, ct)}
                                  onDelete={() => disableContact(empresa, ct)}
                                />
                              ))}
                            </tbody>
                          </table>
            ...
            <div className="flex flex-wrap justify-between items-center gap-2 p-3 text-xs text-gray-600 border-t border-gray-100">
              <span>
                {total > 0 ? `Mostrando ${(page - 1) * PAGE_SIZE + 1} - ${Math.min(page * PAGE_SIZE, total)} de ${total}` : "Sin resultados"}
              </span>
...
function Card({ label, value, loading }) {
  return (
    <div className="bg-white p-4 border rounded-xl">
      ...
      <p className="text-xl font-medium tabular-nums">{loading ? "..." : value}</p>
    </div>
  );
}
