...
// ── Role display ──────────────────────────────────────────────────────────────
const ROL_INFO = {
  adminPlataforma: { label: "Admin Plataforma", badge: "bg-[#1C355E] text-white",     emoji: "💻" },
  adminComercial:  { label: "Admin Comercial",  badge: "bg-[#FFCD00] text-[#1C355E]", emoji: "📊" },
  comercial:       { label: "Asesor Comercial", badge: "bg-emerald-500 text-white",    emoji: "🤝" },
};

// ── Gradients for avatar ──────────────────────────────────────────────────────
const GRADIENTS = [
...
export default function PerfilPage() {
  ...
  const [avatar, setAvatar]   = useState(null); // base64 o null
  ...
  const rolInfo = ROL_INFO[user?.rol] || { label: user?.rol, badge: "bg-gray-100 text-gray-600", emoji: "👤" };
  const initials = (user?.nombre || "U").split(" ").slice(0,2).map((n) => n[0]).join("").toUpperCase();
  const gradient = getGradient(user?.nombre || "");

  // Stats del asesor
  const misVisitas = citas.filter((c) => c.asesorId === user?.id || c.asesorNombre === user?.nombre);
  const stats = {
    total:     misVisitas.length,
    realizadas: misVisitas.filter((c) => c.estado === "realizada").length,
    pendientes: misVisitas.filter((c) => c.estado === "pendiente").length,
  };
    ...
    <LayoutDashboard>
      ...
      <div className="max-w-4xl mx-auto space-y-6 pb-10">
        ...
        <div className="fu bg-white rounded-3xl border border-gray-100 overflow-hidden">
          {/* Banner */}
          <div className="h-28 bg-gradient-to-r from-[#1C355E] via-[#264a82] to-[#1C355E] relative">
              ...
              <div className="flex items-start justify-between flex-wrap gap-3">
                ...
                {!editing ? (
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      onClick={handleSave}
                      className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[#1C355E] text-white text-sm font-bold
                        hover:bg-[#16294d] active:scale-[.97] transition-all"
                    >
                      <SaveIcon /> Guardar
                    </button>
                  </div>
        ...
        {/* STATS – solo para comercial */}
        {user?.rol === "comercial" && (
          <div className="fu fu-1 grid grid-cols-3 gap-3">
            {[
              { label: "Total visitas",    value: stats.total,      color: "bg-[#1C355E]/5 border-[#1C355E]/10", text: "text-[#1C355E]" },
              { label: "Realizadas",       value: stats.realizadas, color: "bg-blue-50 border-blue-100",          text: "text-blue-600" },
              { label: "Pendientes",       value: stats.pendientes, color: "bg-yellow-50 border-yellow-100",      text: "text-yellow-600" },
        ...
        <div className="fu fu-2">
          <SectionCard icon={<UserIcon />} title="Información Personal">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <ProfileField label="Cédula" value={form.cedula}
        ...
        <div className="fu fu-4">
          <SectionCard icon={<ActivityIcon />} title="Actividad Reciente">
            {misVisitas.length === 0 ? (
              <p className="text-sm text-gray-400 text-center py-4">Sin actividad registrada aún</p>
            ) : (
              <div className="space-y-3">
                {misVisitas.slice(-5).reverse().map((v) => (
                  <div key={v.id} className="flex items-center justify-between py-3 border-b border-gray-50 last:border-0">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-400">{v.fecha} · {v.hora}</p>
                      </div>
                    </div>
