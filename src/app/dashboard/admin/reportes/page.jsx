...
export default function ReportesPage() {
  ...
  const hasFilters =
    (isAdmin && filtroAsesor !== "todos") || filtroEstado !== "todos" || filtroDesde || filtroHasta;
      ...
      <main className="flex-1 bg-[#F4F6FA] p-6 md:p-8 min-h-screen">
        ...
        <div className="mb-6 flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="text-xs text-[#98989A] font-medium bg-white border border-gray-200 px-3 py-2 rounded-xl shadow-sm">
            Cumplimiento: <span className="font-black text-[#1C355E]">{cumplimientoPctLabel}%</span> · Finalizadas{" "}
            <span className="font-black text-[#1C355E]">{realizadas}</span> / {goal}
          </div>
        </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              {asesores.map((nombre) => {
                ...
                return (
                  <button
                    key={nombre}
                    onClick={() => handleDownloadAsesor(nombre)}
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-[0_2px_16px_-4px_rgba(28,53,94,0.08)] overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-[#F4F6FA]">
                  {[isAdmin && "Asesor", "Empresa", "Tipo", "Municipio", "Fecha", "Estado"].filter(Boolean).map((h) => (
                    <th key={h} className="text-left px-5 py-3 text-[11px] font-bold text-[#98989A] uppercase tracking-widest whitespace-nowrap">
                      {h}
                    </th>
                  ))}
                </tr>
                ...
                {filtered.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan={isAdmin ? 6 : 5} className="px-6 py-12 text-center text-[#98989A] text-sm">
                      {isAsesor ? "Aún no tienes visitas registradas" : "No hay datos con los filtros actuales"}
                    </td>
                  </tr>
                )}
              </tbody>
