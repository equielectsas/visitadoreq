...
export default function Clientes() {
  ...
  const badge = (src, isProspect) => {
    if (isProspect) return { text: "Prospecto", cls: "bg-amber-100 text-amber-700" };
    if (src === "ERP") return { text: "ERP", cls: "bg-blue-100 text-blue-700" };
  ...
  return (
    <LayoutDashboard>
      <div className="flex justify-between flex-wrap gap-3 mb-5">
        <div>
          ...
          <p className="text-sm text-gray-400">
            {syncStatus
              ? `${syncStatus.total.toLocaleString()} registros · ERP: ${syncStatus.erp.toLocaleString()} · Manuales: ${syncStatus.manual.toLocaleString()}${
                  syncStatus.ultimaSync ? ` · Última sync: ${fmtDate(syncStatus.ultimaSync)}` : ""
                }`
        ...
        <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg text-sm text-blue-800 flex items-center gap-2">
          ...
          <span>
            ...
            {syncStatus && (
              <>
                {" "}
                Total actual: <b>{syncStatus.total.toLocaleString()}</b>
              </>
            )}
          </span>
        </div>
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        <input
          type="text"
          placeholder="Buscar por NIT, nombre, ciudad o dirección..."
          value={search}
                  ...
                  return (
                    <tr key={c._id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3 max-w-[200px]">
                        ...
                        {c.nombrePunto && <div className="text-xs text-gray-400 truncate">{c.nombrePunto}</div>}
                      </td>
                      <td className="px-4 py-3 text-gray-600 whitespace-nowrap">{c.ciudad || "—"}</td>
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        <span className={`px-2 py-1 rounded-md text-xs font-medium ${b.cls}`}>{b.text}</span>
                      </td>
                    </tr>
                  );
                })
...
function NuevoClienteModal({ onClose, onCreated }) {
  ...
  const guardar = async () => {
    if (!form.identificacion.trim()) {
      setAlerta({ tipo: "error", mensaje: "La identificación es obligatoria." });
      return;
    }
    setGuardando(true);
    try {
        ...
        <div className="p-5 space-y-3">
          ...
          <div className="flex gap-2">
            ...
            <input
              ...
              className="border rounded-lg px-3 py-2 text-sm flex-1 focus:outline-none focus:ring-2 focus:ring-blue-200"
            />
          </div>
